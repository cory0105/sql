...
-- 2. 사원 중에서 직위가 '과장'인 사원들의 이름과 직위를 사원번호의 내림차순으로 조회하세요.
select 이름, 직위 from 사원 where 직위='과장' order by 사원번호 desc;
-- 3. 고객번호가 'C001'인 고객의 고객회사명과 담당자명을 조회하세요.
select 고객회사명, 담당자명 from 고객 where 고객번호 = 'CCOPI';
-- 4. 주문일이 2023년 5월 1일 이후인 주문의 주문번호와 주문일을 최신 주문일 순으로 조회하세요.
select 주문번호, 주문일 from 주문 where 주문일 > '2021-05-01' order by 주문일 desc;
...
-- 7. 고객의 마일리지가 100 이상인 고객들의 고객회사명과 마일리지를 조회하세요. 결과는 마일리지가 높은 순으로 정렬하세요.
select 고객회사명, 마일리지 from 고객 where 마일리지>=100 order by 마일리지 desc;
