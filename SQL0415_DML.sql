...
-- insert into 테이블명 values(값1, 값2, ...); -- 컬럼 순서대로 작성해야 함
insert into 부서 values('A5', '마케팅부');
insert into 제품 values(91, '연어피클소스', null, 5000, 40);

-- insert into 테이블명(컬럼1, 컬럼2, ...) values(값1, 값, ...); -- 이 경우 values는 위의 컬럼 순서대로 써야 함
...
update 사원 set 이름 = '김레몬' where 사원번호 = 'E20'; -- where 조건이 없으면 모든 열이 변경됨
update 제품 set 포장단위 = '200 ml bottles' where 제품번호 = 91;
update 제품 set 단가 = 단가 * 1.1, 재고 = 재고 - 10 where 제품번호 = 90;

-- delete from 테이블명 where 조건;
delete from 제품 where 제품번호 = 91;

-- 가장 최근 입사자 순서로 정렬 후 위에서 3개 row 삭제
delete from 사원 order by 입사일 desc limit 3;

-- insert on duplicate key update
-- 레코드(=열, row, 튜플)가 없으면 새롭게 추가하고, 있으면 변경
insert into 제품(제품번호, 제품명, 단가, 재고) values(91, '연어피클핫소스', 6000, 50) 
