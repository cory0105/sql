...
select *, concat('**',substr(고객회사명, 3)) as 고객회사명2, substr(replace(전화번호, ')', '-'), 2) as 전화번호2 from 고객;

/* 2. 주문세부 테이블에서 아래 3개의 컬럼을 조건대로 만드시오 / 소수점 아래 자리는 1자리에서 버림(truncate)
	주문금액 : 주문수량 * 단가
	할인금액 : 주문수량 * 단가 * 할인율
	실주문금액 : 주문금액 - 할인금액 */
select 주문수량 * 단가 as 주문금액, truncate(주문수량 * 단가 * 할인율, 1) as 할인금액, (주문수량 * 단가) - truncate(주문수량 * 단가 * 할인율, 1) as 실주문금액 from 주문세부;

/* 3. 사원테이블에서 전체사원의 이름, 생일, 만나이(as 만나이), 입사일, 입사일수(as 입사일수), 입사한지 500일후(as 입사500일후)의 날짜를 만드시오 */
select 이름, 생일, timestampdiff(year, 생일, now()) as 만나이, 입사일, concat('입사', timestampdiff(day, 입사일, now()), '일후') as 입사일,
...
case when 마일리지>=100000 then 'VVIP'
	...
	else '일반고객' end as 마일리지구분 from 고객;
...
case when weekday(주문일) = 0 then '월요일'
	...
	when weekday(주문일) = 6 then '일요일' end as 한글요일 from 주문;
...
case weekday(주문일) when 0 then '월요일'
	...
	when 4 then '금요일'
	when 5 then '토요일'
	when 6 then '일요일' end as 한글요일 from 주문;
