...
select substring_index('서울시 동작구 흑성동', ' ', -1);   
select substring_index(substring_index('서울시 동작구 흑성동', ' ', 2), ' ',-1);

-- lpad 지정한 길이에서 문자열을 제외한 나머지를 특수문자로 왼쪽부터 채움
-- rpad 지정한 길이에서 문자열을 제외한 나머지를 특수문자로 오른쪽부터 채움
select lpad('sql',10,'#'),
	   rpad('sql',10,'#');
...
-- ltrim 왼쪽 공백 제거
-- rtrim 오른쪽 공백 제거
select trim('   sql     '), ltrim('    sql     '), rtrim('     sql        ');
