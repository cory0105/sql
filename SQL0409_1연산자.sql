...
select * from 고객 where 도시 = '부산광역시' or 마일리지<1000;
select * from 고객 where not 도시 = '서울특별시';

-- 집합연산자 (union, union all)
-- 1) 첫번째와 두번째 select문의 컬럼 갯수 동일
-- 2) 컬럼이 다르다면 데이터타입은 같아야함
-- union all은 중복허용
...
-- 특수문자 %, _ 사용
select * from 고객 where 도시 like '%광역%';
select * from 고객 where 고객번호 like '_C%';
select * from 고객 where 고객번호 like '__C%';

-- 전화번호 뒷자리가 45로 끝나는 고객
select * from 고객 where 전화번호 like '%45';
-- 전화번호 중 뒤에서 3번째부터가 98인 고객
...
select * from 제품 where 제품번호 in(1,2,3,7,11,20);
